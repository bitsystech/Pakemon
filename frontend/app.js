document.addEventListener('DOMContentLoaded', async () => {
    ...
    uploadForm.addEventListener('submit', async (e) => {
        ...
        // Handle checkboxes which might not be in formData if unchecked
        ['InstallBehavior_ShowProgress', 'InstallBehavior_AllowDefer', 'Requirements_RequireAdmin', 'Scripts_EnablePreScript', 'Scripts_FailIfPreScriptFails', 'Scripts_EnablePostScript'].forEach(cb => {
            const el = uploadForm.elements[cb];
            if (el) {
                configPayload[cb] = el.checked;
            }
        });
    ...
    async function loadApps(retryCount = 3) {
        try {
            ...
            if (retryCount > 0) {
                console.log(`Retrying loadApps... (${retryCount} attempts left)`);
                setTimeout(() => loadApps(retryCount - 1), 1000);
            }
        }
    }

    async function loadRequests(retryCount = 3) {
        try {
      ...
      `;
            ...
            if (retryCount > 0) {
                console.log(`Retrying loadRequests... (${retryCount} attempts left)`);
                setTimeout(() => loadRequests(retryCount - 1), 1000);
            }
        }
    }
